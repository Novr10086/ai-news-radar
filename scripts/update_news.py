...
def fetch_iris(session: requests.Session, now: datetime) -> list[RawItem]:
    ...
    out: list[RawItem] = []
    import socket as _iris_sock
    for feed_name, feed_url in feeds:
        if len(out) >= DISCUSSION_FETCH_CAP:
            break
        try:
            _iris_sock.setdefaulttimeout(15)
...
def fetch_wesearch(session: requests.Session, now: datetime) -> list[RawItem]:
    """Fetch world news from WeSearch (wesearch.press) via obscura headless browser."""
    site_id = "wesearch"
    site_name = "WeSearch"

    # Requires a local headless-browser helper (OBSCURA_SCRIPT env var);
    # skip this source when it is not configured.
    obscura_script = os.environ.get("OBSCURA_SCRIPT", "")
    if not obscura_script:
        return []
    url = "https://wesearch.press/world-news"

    try:
        result = subprocess.run(
            [obscura_script, url, "--markdown"],
            capture_output=True,
            text=True,
            timeout=60,
    ...
    if not markdown or "Today's stories" not in markdown:
        return []

    # Parse markdown: extract articles from the "Today's stories" section
    # Pattern: [SOURCE\n\n### TITLE\n\nTIMESTAMP · N views](URL)
    today_idx = markdown.index("Today's stories")
    after_today = markdown[today_idx:]

    # Find article blocks: [SOURCE\n\n### TITLE\n\nTIMESTAMP · N views](URL)
    ...
    for match in article_pattern.finditer(after_today):
        source = match.group(1).strip()
        title = match.group(2).strip()
        meta_text = match.group(3).strip()
        ...
        # Parse timestamp from meta_text e.g. "Sun, 05 Jul 2026 00:53:53 GMT · 6 views"
        published = None
        ts_match = re.search(
            r"(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun),\s*\d{1,2}\s+\w+\s+\d{4}\s+\d{2}:\d{2}:\d{2}\s+GMT",
            meta_text,
        )
        if ts_match:
            try:
                published = dtparser.parse(ts_match.group(0)).replace(tzinfo=timezone.utc)
            except Exception:
                pass
...
def collect_all(session: requests.Session, now: datetime) -> tuple[list[RawItem], list[dict[str, Any]]]:
    tasks = [
        ...
        ("wesearch", "WeSearch", fetch_wesearch),
    ]
